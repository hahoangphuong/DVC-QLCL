...
workflow_base AS (
  SELECT
    ...
    cf.submission_kind,
    CASE
      WHEN w.thu_tuc = 48 AND (w.buoc = 'chua_xu_ly' OR w.don_vi = 'Ph�ng ban ph�n c�ng') THEN 'chua_xu_ly'
      WHEN w.thu_tuc = 48 AND w.buoc = 'bi_tra_lai' THEN 'bi_tra_lai'
      WHEN w.thu_tuc = 48 AND w.buoc = 'cho_tong_hop' THEN 'cho_tong_hop'
      WHEN w.don_vi = 'Chuy�n gia th?m d?nh' THEN 'cho_chuyen_gia'
      WHEN w.don_vi = 'T? tru?ng chuy�n gia' THEN 'cho_to_truong'
      WHEN w.don_vi = 'Tru?ng ph�ng' THEN 'cho_truong_phong'
      WHEN w.buoc = 'cho_ket_thuc' OR w.don_vi IN ('Ph� C?c tru?ng', 'Van thu') THEN 'cho_cong_bo'
      WHEN w.buoc IN ('chua_xu_ly', 'bi_tra_lai', 'cho_tong_hop')
        OR w.don_vi IN ('Chuy�n vi�n', 'Ph�ng ban ph�n c�ng')
    ...
    END AS chuyen_vien,
    CASE
      WHEN w.don_vi = 'Chuy�n gia th?m d?nh' AND NULLIF(TRIM(w.nguoi_xu_ly), '') IS NOT NULL
