...
- that reuse should include the `renderTabContent` callback signature as well, so `DashboardRuntimePanels.tsx` does not re-import `DashboardTabId` just to restate the same contract
...
- `features/layout/DashboardRuntimePanels.tsx` should also export its full panel-host prop contract, and `DashboardRuntime.tsx` should prefer passing one typed `runtimePanelsProps` object instead of expanding the panel prop list inline
...
- `features/layout/useDashboardSyncStatus.ts` should also accept `DashboardRole | null`, not a looser `string | null` auth-role contract
